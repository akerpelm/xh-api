const doProcessTrades = (tradeData) => {
  ...
  let faultyTradesToRemoved = [...tradeData]; //Should not directly manipulate input
  for (let individualTrade of faultyTradesToRemoved) {
    if (individualTrade.id == "00000000-0000-0000-0000-000000000000") {
      ...
      faultyTradesToRemoved.splice(
        faultyTradesToRemoved.indexOf(individualTrade),
        1
      );
    }
  }

  const sanitizedTradeData = faultyTradesToRemove; //once faultyTrades are spliced, iterate over remaining:
